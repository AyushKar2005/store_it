...
const createQueries = (currentUser: Models.Document,types:string[],searchText:string,sort:string,limit?:number) => {
    ...
    if (sort) {
        ...
        const validSortFields = ["name", "type", "size", "extension"]; // only schema attributes!
...
// eslint-disable-next-line no-undef
export const getFiles = async ({types=[],searchText="",sort="name-desc",limit}:GetFilesProps) => {
